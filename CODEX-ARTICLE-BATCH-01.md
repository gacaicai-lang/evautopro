...
4–6 张正文图（不含 hero，按 eyouagro 节奏约每 300–500 词一张）、≥1 个 `/cars/` 产品页内链、2–3 个 `related`。
...
| 10 | geely-coolray-vs-changan-cs55-plus-export | PUBLISHED | 2026-07-16 | see git history | cars.ts only; RHD MOQ/lead time explicit |

状态值：`TODO` → `RESEARCHING` → `PUBLISHED` →（Claude 填）`REVIEWED`

BATCH COMPLETE + 2026-07-12
